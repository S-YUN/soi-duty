...
## 후속 수정 (2026-09-17 저녁, 사용자 피드백)

- **퇴근 시각 계산을 "남은 근무일 균등 배분"으로 교체.** CLAUDE.md "오늘 퇴근 시각 계산"이 정본. 오늘 진행분은 남은 시간에서 빼지 않는다(제가 정함 — 넣으면 일할수록 몫이 줄어 시각이 흔들린다).
- **안내 문구** 표(CLAUDE.md "안내 문구"). 출근 전·근무 중 공통. 반차인 날은 "페이스 좋아요"로(4h 고정 몫을 8h와 비교하면 항상 "일찍"이 떠서). 마지막 근무일이 금요일이 아니면 "이번 주 마지막 날! 오늘도 힘내세요".
- **근무 중 상태 블록**: "● 09:12 출근"(20/w600) / "7시간 15분째 근무중 · 18:12 퇴근 추천"(13.5) / 안내 문구(12.5). 채웠으면 "· 이번 주 이미 채웠어요". 출근 전은 "● 아직 출근 전" / 안내 문구.
- 알약 탭: 위 여백 8→14, 세로 패딩 9→7, 글자 13.5→13.
- 캘린더: "기준 대비 ±" 헤더 삭제, 반차 날은 배지 + 기준 대비, 셀 높이 58→68. 근무한 주말 셀에도 배경.
- 시트 칩 선택 시 ✓ 아이콘. **미래 날짜 시트는 칩 탭 한 번으로 저장·닫힘**, 취소/저장 버튼 없음(기록 있으면 "기록 지우기"만).
- 스플래시는 보류(사용자가 시안 뽑아둠). 백업/복원은 다음 라운드. 월간 리포트는 온보딩성 안내로 나중에.

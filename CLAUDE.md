...
- 출퇴근 중 하나라도 비어 있는 평일.
- 탐색 범위는 **첫 기록일부터 어제까지.** 앱 설치 전 날짜는 대상이 아니다. 첫 기록일이 아직 없으면 오늘을 첫 기록일로 간주한다.
- **첫 주만 예외로 그 주 월요일부터.** 수요일에 설치했으면 첫 주 동안은 월·화가 환영 카드 아래 "기록 안 된 날"에 떠서
  채워볼 수 있다(채우면 첫 기록일이 당겨져 첫 주 예외가 풀린다). 그 주가 지나면 그 날들은 설치 전 날짜로 취급해 더 조르지 않는다 —
  첫 주는 목표가 없어 어떤 계산에도 쓰이지 않기 때문이다. 주간·월간에서 편집은 여전히 가능하다 (2026-09-18).
- 주말은 대상이 아니다.
...
- **근무 중엔 "출근 변경"으로 출근 시각만 고친다.** 출근하고 한참 뒤에 찍은 걸 바로잡는 용도의 작은 시트
  (제목 → 휠 → 저장). 지금보다 늦은 시각은 저장하지 않는다. 근무 중 보조 슬롯은
  "☐ 오늘 반차 · 출근 변경 · 출근 취소" 셋 (2026-09-18).
...
드라이버는 캘린더 셀을 `calendarCellKey`(= `cell-2026-9-24`)로 집고 날짜는 실행 시점에 고른다 — 날짜 숫자를 하드코딩하면 오늘이 바뀔 때 깨진다.
...
- **월간 캘린더**: 셀 값은 얇은 공백(U+2009)·비례 숫자·좁은 자간으로 "+3h 52m"이 44px 칸에 원래 크기로 들어간다
  (`formatNarrowHm`, `AppTextStyles.calendarValue`). 셀 폭이나 글꼴을 바꾸면 다시 측정한다.
  날짜 숫자 뒤 **원 색이 곧 유형**(범례와 같은 색). 일반 근무일은 주말 포함 `#EFF1EC` 하나.
